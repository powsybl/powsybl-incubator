...
subject to Active_power_balance{i in BUSES: i != slack_bus}:
    ...
    == (sum {j in GENS: gen_bus[1,j] == i} gen_targetp[1,j] - sum {j in LOADS: ld_bus[1,j] == i} ld_p0[1,j])/100;
...
subject to Rective_power_balance_PQ{i in PQ_BUSES}:
    ...
    == (sum {j in GENS: gen_bus[1,j] == i} gen_targetq[1,j] - sum {j in LOADS: ld_bus[1,j] == i} ld_q0[1,j])/100;
...
subject to Reactive_power_inf{i in PV_BUSES}:
    sum {j in GENS: gen_bus[1,j] == i} gen_minq0[1,j]/100 <= Q[i];
    
subject to Reactive_power_sup{i in PV_BUSES}:
    Q[i] <= sum {j in GENS: gen_bus[1,j] == i} gen_maxq0[1,j]/100;
